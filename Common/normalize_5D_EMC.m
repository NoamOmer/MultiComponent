...
function sim_mat = normalize_5D_EMC(sim_mat)
for B0_idx = 1:size(sim_mat,1)
for B1_idx = 1:size(sim_mat,2)
for T1_idx = 1:size(sim_mat,3)
for T2_idx = 1:size(sim_mat,4)
	tmp = squeeze(sim_mat(B0_idx,B1_idx,T1_idx,T2_idx,:));
	tmp = tmp / tmp(1);
	if (sum(tmp<0))
		error('EMC error');
	end;
	sim_mat(B0_idx,B1_idx,T1_idx,T2_idx,:) = tmp;
end;
end;
end;
end;