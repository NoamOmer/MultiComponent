function params=setParams()
    ...
    % ----------------------------------------------------------------------------------------
    params.nExprT2                  = 3;					% # of T2 components in synthesized vector
    params.peakWidth                = 1;		
    params.mixMode                  = 1;					% [1: Series of single T2 values | 2: Series of gaussians distribution around each T2]
    params.loc					    = [ 23   ,33  ,89	];	% [ms]		T2 components at location 'loc'
    params.relativeFract		    = [ 0.5	 ,0.4  ,0.1	];	% [none]	relative fraction of each component
    ...
    params.snr                      = 100;					% 0: no noise. MRI typical SNR 30-100.
