classdef Simulator
    ...
    methods    
        ...
        function obj=loadSimData(obj,dictionary)
			
			% ----------------------------------------------------------------------
			% Load EMC data from a saved database (i.e. a '.mat' file)
			% ---------------------------------------------------------------------- 
			if nargin==1
				% Nz=4000, EMC_Abs_flag=1, TE=12, ETL=40, RefAng=120, BW=200
				simData	= load('C:\Users\me\Google Drive\Noam Omer\MultiCompT2\DB\2016_09_22_T2_1_1000_B1_100_for_MultiT2Fit_400um_L_20mm_ABS.mat');
			else
				rootDir=cd;
				cd ..
				cd DB
				simData = load(dictionary.DPpath);
				cd(rootDir);
			end
			...
			B1_1						= dictionary.params.B1; % find(simData.B1_scaling_arr==1); % here was the BUG!!!!! [NO]
        ...
        function obj=simulate(obj,type)
			...
			elseif strcmp(type,'exprmt')
				...
				axisT2						= axis_T2_full;
				n_T2s					    = size(emcT2_full,4); 
				emcT2_sprs					= emcT2_full;
				
				[E,E_noisy]=preprocess_emc(axisT2,emcT2_sprs,firstTE,snr,denoise_flag,ETL,type);
				% Do not include E in the analysis - only for plotting purposes ! 
        ...
        function obj=genrtExpEMC(obj)
			...
			obj.experimental.experimental_denoised_emc	= experimental_denoised_emc;
